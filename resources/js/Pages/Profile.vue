...
const getSchools = () => {
  const school_id = props.Schools.find(s => s.short_name === school.short_name?.toUpperCase())?.value
  if (!school.short_name || !school_id) {
    school.error = "Not a valid school code"
    school.id = null;
  } else {
    school.id = school_id;
    school.error = "";
...
<template>
  <LayoutAuthenticated>
    <SectionMain>
      ...
      <div v-if="isTeacher()" id="update-school" class="grid grid-cols-1 mt-6">
      ...
      <div v-if="isStudent()" id="update-school" class="grid grid-cols-1 mt-6">
        <CardBox is-form @submit.prevent="getSchools">
          <SectionTitleLineWithButton
              :icon="mdiAccountEdit"
              title="Get School"
          ...
          <div class="flex gap-4">
            ...
            <BaseButtons class="mb-0">
              <BaseButton class=" h-12" :disabled="!school.short_name" color="info" type="submit" label="Get Schools"/>
            </BaseButtons>
      ...
      <div v-if="isStudent() && school.id" id="update-school" ref="targetElement" class="grid grid-cols-1 mt-6">
        <CardBox is-form @submit.prevent="updateSchool">
          <SectionTitleLineWithButton
              :icon="mdiAccountEdit"
              title="Update School"
